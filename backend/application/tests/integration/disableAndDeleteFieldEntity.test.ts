...
describe('Entity Cascading Deletion Integration Test', () => {
    ...
    it('should delete a thesis without affecting parent entities', async () => {
        ...
        const thesis = await table(db, 'theses')
            .where('id', TEST_DELETE_THESIS_ID)
            .first();

        expect(thesis).toBeUndefined();

        const thesisSector = await table(db, 'theses_in_sectors')
            .where('thesis_id', TEST_DELETE_THESIS_ID)
            .first();

        expect(thesisSector).toBeUndefined();

        const thesisDeviceAssociation = await table(db, 'theses_devices')
            .where('thesis_id', TEST_DELETE_THESIS_ID)
            .first();

        expect(thesisDeviceAssociation).toBeUndefined();

        const device = await table(db, 'devices')
            .where('id', TEST_DELETE_DEVICE_ASSOCIATED_ID)
            .first();

        expect(device).toBeDefined();

        const advice = await table(db, 'advices')
            .where('thesis_id', TEST_DELETE_THESIS_ID)
            .first();

        expect(advice).toBeUndefined();

        const wateringParams = await table(db, 'watering_algorithm_params')
            .where('thesis_id', TEST_DELETE_THESIS_ID)
            .first();

        expect(wateringParams).toBeUndefined();

        const sector = await table(db, 'sectors')
            .where('id', TEST_DELETE_SECTOR_ID)
            .first();
    ...
    it('should disable a sector and its associations', async () => {
        ...
        await request(app)
            ...
            .set('Authorization', `Bearer ${authToken}`)
            .send({
                targetId: TEST_DELETE_SECTOR_ID,
                targetType: DeviceTargetType.SECTOR,
                validFrom: validTo - 1000,
            })
            .expect(200);
     ...
     */
    it('should delete a sector and its related entities', async () => {
        expect(
            await table(db, 'sectors')
                .where('id', TEST_DELETE_SECTOR_ID)
                .first(),
        ...
        const thesisId = (
            await table<{ thesis_id: number }>(db, 'theses_in_sectors')
                .where('sector_id', TEST_DELETE_SECTOR_ID)
                .first()
        )?.thesis_id;
        ...
        const sectorDevice = await table(db, 'sectors_devices')
            .where('sector_id', TEST_DELETE_SECTOR_ID)
            .first();
        ...
        const wateringEvent = await table(db, 'watering_events')
            .where('sector_id', TEST_DELETE_SECTOR_ID)
            .first();

        expect(wateringEvent).toBeUndefined();

        const sectorService = await table(db, 'sectors_services')
            .where('sector_id', TEST_DELETE_SECTOR_ID)
            .first();
    ...
    it('should disable a farm and its associations', async () => {
        ...
        await request(app)
            ...
            .set('Authorization', `Bearer ${authToken}`)
            .send({
                targetId: TEST_DELETE_FARM_ID,
                targetType: DeviceTargetType.FARM,
                validFrom: validTo - 1000,
            })
            .expect(200);
    ...
    it('should delete a farm and cascade deletion to its sectors and their theses', async () => {
        ...
        const sectorId = (
            await table<{ id: number }>(db, 'sectors')
                .where('farm_id', TEST_DELETE_FARM_ID)
                .first()
        )?.id;
        ...
        const farm = await table(db, 'farms')
            .where('id', TEST_DELETE_FARM_ID)
            .first();
        ...
        const farmDevice = await table(db, 'farms_devices')
            .where('farm_id', TEST_DELETE_FARM_ID)
            .first();
    ...
    it('should disable a company and its associations', async () => {
        ...
        await request(app)
            .post(`/companies/${TEST_DELETE_COMPANY_ID}/disable`)
            .query({ validTo })
            .set('Authorization', `Bearer ${authToken}`)
            .expect(200);
        ...
        expect(
            await table(db, 'farms')
                .where('company_id', TEST_DELETE_COMPANY_ID)
                .andWhere(function () {
                    this.where('disabled_at', '>', validTo + 1)
                        .orWhereNull('disabled_at');
                })
                .first(),
        ).toBeUndefined();
    ...
    it('should delete a company and all its associated child entities', async () => {
        ...
        const farmId = (
            await table<{ id: number }>(db, 'farms')
                .where('company_id', TEST_DELETE_COMPANY_ID)
                .first()
        )?.id;
        ...
        const thesisId = (
            await table<{ id: number }>(db, 'theses_in_sectors')
        ...
        const company = await table(db, 'companies')
            .where('id', TEST_DELETE_COMPANY_ID)
            .first();
