...
describe('Optimal State Assignment Integration Test', () => {
    ...
    it('should assign an existing optimal to a thesis', async () => {
        await request(app)
            ...
            .query({optimalProfileId: TEST_EXISTING_OPTIMAL_STATE, validFrom: validFrom})
            .send({ stopThreshold: STOP_THRESHOLD, optimalWetBound: WET_BOUND, optimalDryBound: DRY_BOUND })
            .expect(200)

        const thesisGrid = await table(db, 'theses_all_signals')
            ...
            .select('device_id').first();
        expect(thesisGrid.device_id).toBeDefined()
        const record = await table(db, 'grid_optimal_profile_assignment')
            .where('grid_id', thesisGrid.device_id)
            .andWhere('optimal_profile_id', TEST_EXISTING_OPTIMAL_STATE)
            .andWhere(function () {
                this.where('valid_to', '>', validFrom).orWhereNull('valid_to')
            })
            .andWhere('valid_from', validFrom)
        ...
        const oldAssignements = await table(db, 'grid_optimal_profile_assignment')
            .where('grid_id', thesisGrid.device_id)
            .andWhere(function () {
                this.where('valid_to', '>', validFrom + 1).orWhereNull('valid_to')
            })
            .andWhere('valid_from', '<', validFrom + 1)
        expect(oldAssignements).toHaveLength(1)
    })
    ...
    it('should assign an existing interpolated profile to a thesis', async () => {
        ...
        await request(app)
            .put(`/theses/${TEST_THESIS_ID}/setOptimalState`)
            .set('Authorization', `Bearer ${authToken}`)
            .query({thesisId: TEST_IMAGE_THESIS_ID, imageTimestamp: TEST_IMAGE_TIMESTAMP, validFrom: profileValidFrom})
            .send({ stopThreshold: STOP_THRESHOLD, optimalWetBound: WET_BOUND, optimalDryBound: DRY_BOUND })
            .expect(200)
        ...
        const thesisGrid = await table(db, 'theses_all_signals')
            ...
            .andWhere('device_type', 'SOIL_MOISTURE_GRID')
            .andWhere(function () {
                this.where('valid_to', '>', profileValidFrom).orWhereNull('valid_to')
            })
            .andWhere('valid_from', '<', profileValidFrom)
            .select('device_id').first();
        expect(thesisGrid.device_id).toBeDefined()
        const undefinedOptimal = await table(db, 'grid_optimal_profile_assignment')
            .where('grid_id', thesisGrid.device_id)
            .andWhere(function () {
                this.where('valid_to', '>', profileValidFrom - 1).orWhereNull('valid_to')
            })
            .andWhere('valid_from', profileValidFrom - 1)
            .first()
    ...
    it('should assign a custom profile to a thesis', async () => {
        ...
        await request(app)
            .put(`/theses/${TEST_THESIS_ID}/setOptimalState`)
            .set('Authorization', `Bearer ${authToken}`)
            .query({validFrom: profileValidFrom})
            .send({ stopThreshold: STOP_THRESHOLD, optimalWetBound: WET_BOUND, optimalDryBound: DRY_BOUND, optimalProfile: OPTIMAL_PROFILE })
            .expect(200)
