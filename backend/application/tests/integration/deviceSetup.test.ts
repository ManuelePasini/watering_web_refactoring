...
describe('Device and Signal Setup Integration Test', () => {
    ...
    let deviceId: number;
    let tempSignalId: number;
    let humiditySignalId: number;
    let windSignalId: number;
    ...
    const PROVIDER_ID = 1;
    const SIGNAL_TYPE_TEMP = 4;
    const SIGNAL_TYPE_HUM = 5;
    const SIGNAL_TYPE_WIND = 8;
    ...
    it('should create a Device and persist it', async () => {
        ...
        const record = await table<{ type: string; created_at: number, location_json: string }>(db, 'devices')
            .select(
                '*',
                db.raw('public.ST_AsGeoJSON(location) as location_json'),
            )
            .where('id', deviceId)
            .first();
    ...
    it('should create an Air Temperature Signal', async () => {
        ...
        tempSignalId = Number(res.body.id);

        const record = await table<{ provider_id: number; type_id: number; id_on_provider: string }>(db, 'signals')
            .where('id', tempSignalId)
            .first();
    ...
    it('should retrieve the Device and see connected signals', async () => {
        ...
        const signalIds: number[] = res.body.signals.map(
            (signal: { id: number }) => signal.id,
        );
    ...
    it('should disable a Device and check all signals are disconnected', async () => {
        ...
        await table(db, 'devices_signals')
            .where('device_id', TEST_DELETE_DEVICE_ID)
            .andWhere('valid_from', '<', validTo + 1)
            .andWhere(function () {
                this.where('valid_to', '>', validTo + 1)
                    .orWhereNull('valid_to');
        ...
        await table(db, 'grid_optimal_profile_assignment')
            .where('grid_id', TEST_DELETE_DEVICE_ID)
            .andWhere('valid_from', '<', validTo + 1)
            .andWhere(function () {
                this.where('valid_to', '>', validTo + 1)
                    .orWhereNull('valid_to');
        ...
        await table(db, 'theses_all_signals')
            .where('device_id', TEST_DELETE_DEVICE_ID)
            .andWhere('valid_from', '<', validTo + 1)
            .andWhere(function () {
                this.where('valid_to', '>', validTo + 1)
    ...
    it('should delete a device and all related data', async () => {
        const signalIds: number[] = (
            await table<{ signal_id: number }>(db, 'devices_signals')
                .where('device_id', TEST_DELETE_DEVICE_ID)
        ).map(
            ds => ds.signal_id,
        );
        ...
        const signalAssociations = signalIds.length > 0
            ? await table<{ signal_id: number; device_id: number }>(db, 'devices_signals')
                .whereIn('signal_id', signalIds)
                .distinct('signal_id', 'device_id')
                .select('signal_id', 'device_id')
            : [];

        const signalDeviceCounts = signalAssociations.reduce<Record<number, number>
        >((counts, association) => {
            counts[association.signal_id] =
                (counts[association.signal_id] || 0) + 1;
        ...
        expect(remainingSharedSignals)
            .toHaveLength(sharedSignalIds.length);
