...
const apiSpec = swaggerSpec as Parameters<typeof OpenApiValidator.middleware>[0]['apiSpec']
...
app.use(
  '/users',
  (usersRouter as any)({ userService, authenticationService, authorizationService })
)

app.use(
  '/farms',
  (farmsRouter as any)({ userService, authenticationService, authorizationService, fieldService })
)

app.use(
  '/sectors',
  (sectorsRouter as any)({ userService, authenticationService, authorizationService, fieldService, sectorServicesService })
)

app.use(
  '/theses',
  (thesesRouter as any)({ authenticationService, authorizationService, fieldService, wateringAdviceService })
)

app.use(
  '/organizations',
  (organizationsRouter as any)({ organizationService, authenticationService, userService, authorizationService })
)

app.use(
  '/companies',
  (companiesRouter as any)({ companyService, userService, authenticationService, authorizationService })
)
...
app.use(
  '/signals',
  (signalsRouter as any)({ authenticationService, authorizationService, signalService })
)

app.use(
  '/fieldCharts',
  (fieldChartRouter as any)({ authenticationService, authorizationService, fieldService })
)

app.use(
  '/profileBins',
  (profileBinsRouter as any)({ authenticationService, authorizationService, fieldService })
)

app.use(
  '/wateringSchedule',
  (wateringScheduleRouter as any)({ authenticationService, authorizationService, wateringScheduleService, fieldService })
)

app.use(
  '/logs',
  (logsRouter as any)({ authenticationService, authorizationService, logService, fieldService })
)

app.use(
  '/services',
  (sectorServicesRouter as any)({ authenticationService, authorizationService, sectorServicesService })
)
...
const validationErrorHandler: ErrorRequestHandler = (error: OpenApiValidationError, req, res, next) => {
  if (error.status && error.errors) {
    ...
    const simplifiedErrors = error.errors.map(e => ({
      path: e.path,
      message: e.message
    }))
